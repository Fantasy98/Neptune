...
from utils.newnet import FCN_Pad_Xaiver_CBAM_InOut2
...
model =FCN_Pad_Xaiver_CBAM_InOut2(HEIGHT,WIDTH,CHANNELS,KNSIZE,padding)
model.apply(Init_Conv)
if torch.cuda.device_count() > 1:
    print("There are ", torch.cuda.device_count(), "GPUs!")
    print(f"Device: {device_ids} will be used !")
    model = DataParallel(model,device_ids=device_ids)
model.to(device)

model_name = "CBAM_InOut"
...
EPOCH = 100
model_dir = Name_Checkpoint(y_plus,var,target,normalized,EPOCH,model_name=model_name)
...
config= {"batch_size":batch_size,
        ...
        "lr":1e-3}
import json
with open("23-1-18-config_{}.json".format(model_name),"w") as f:
